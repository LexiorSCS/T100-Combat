...
export class HealthBlocksUI {
    private scene: CombatUI;
    private healthBlocks: Map<string, Phaser.GameObjects.Container> = new Map();

    constructor(scene: CombatUI) {
        this.scene = scene;
    }

    updateHealthBlocks(unit: Unit) {
        // Remove existing container if it exists
        this.removeHealthBlocks(unit);

        // Create new container
        const container = this.scene.add.container(unit.container.x, unit.container.y);
        ...
        this.healthBlocks.set(unit.name, container);
    }

    updatePosition(unit: Unit) {
        const container = this.healthBlocks.get(unit.name);
        if (container) {
            container.setPosition(unit.container.x, unit.container.y);
        }
    }

    removeHealthBlocks(unit: Unit) {
        const container = this.healthBlocks.get(unit.name);
        if (container) {
            container.destroy();
            this.healthBlocks.delete(unit.name);
        }
    }
}
...
export default class CombatUI extends Phaser.Scene {
    ...
    private healthBlocksUI: HealthBlocksUI;
    ...
    create() {
        ...
        this.healthBlocksUI = new HealthBlocksUI(this);
        
        // Add listener for health block updates
        this.events.on('updateHealthBlocks', (unit: Unit) => {
            this.healthBlocksUI.updateHealthBlocks(unit);
        });

        // Add listener for unit movement
        this.events.on('updateHealthBlockPositions', (unit: Unit) => {
            this.healthBlocksUI.updatePosition(unit);
        });

        // Listen for unit position updates
        this.events.on('updateUnitPosition', (unit: Unit) => {
            this.healthBlocksUI.updatePosition(unit);
        });
